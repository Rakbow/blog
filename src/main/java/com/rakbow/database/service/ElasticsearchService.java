...
import com.rakbow.database.dao.elasticsearch.AlbumRepository;
...
public class ElasticsearchService {
    ...
    public List<Album> searchAlbum(String keyword, int limit) throws IOException, ParseException {
        ...
        for(SearchHit hit : searchResponse.getHits()){
            ...
            // 处理高亮显示的结果
            HighlightField nameZhField = hit.getHighlightFields().get("nameZh");if(nameZhField != null){
            ...
            HighlightField nameEnField = hit.getHighlightFields().get("nameEn");if(nameEnField != null){
                album.setNameEn(nameEnField.getFragments()[0].toString());
            }
            HighlightField labelField = hit.getHighlightFields().get("label");if(labelField != null){
                album.setLabel(labelField.getFragments()[0].toString());
            }
            HighlightField publisherField = hit.getHighlightFields().get("publisher");if(publisherField != null){
                album.setPublisher(publisherField.getFragments()[0].toString());
            }
            HighlightField distributorField = hit.getHighlightFields().get("distributor");if(distributorField != null){
                album.setDistributor(distributorField.getFragments()[0].toString());
            }
            HighlightField copyrightField = hit.getHighlightFields().get("copyright");if(copyrightField != null){
